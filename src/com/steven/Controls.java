...
public class Controls extends AbstractView implements EventHandler<MouseEvent>{
    ...
    protected void initView() {
        ...
        current_time = LabelBuilder.create().textFill(Color.WHITE).text("00:00").layoutX(200).layoutY(3.0).build();
        ...
        time.setDragingHandler(new DragingHandler() {
            ...
            public void dragHandler(double progress) {
                ...
                if (ViewsContext.player().getStatus() == MediaPlayer.Status.STOPPED) {
        ...
        prev = LabelBuilder.create().prefHeight(45).prefWidth(168 / 4).id("prev").onMouseClicked(this).layoutX(81)
                .layoutY(50).build();
        root.getChildren().add(prev);

        play_pause = LabelBuilder.create().prefHeight(45).prefWidth(336 / 8).id("play").onMouseClicked(this).layoutX
                (123).layoutY(50).build();
        root.getChildren().add(play_pause);

        next = LabelBuilder.create().prefHeight(45).prefWidth(168 / 4).id("next").onMouseClicked(this).layoutX(165)
                .layoutY(50).build();
        ...
        Button lyric = ButtonBuilder.create().id("lyric_btn").layoutX(5).layoutY(74).build();
        root.getChildren().add(lyric);
        ...
        sound = new Zlider(61, 12, 0, ImageUtils.split(ResourceManager.loadClasspathImage("sound_thumb.png"), 4)
                [0]);
        sound.setId("sound");
        sound.setLayoutY(78);
        ...
        ViewsContext.player().currentTimeProperty().addListener(new InvalidationListener() {
            ...
            public void invalidated(Observable observable) {
                ...
                if (music.time == null || duration == null) {
                    ...
                    double vlaue = duration.toMillis() / music.time.toMillis();
                    time.setProgress(vlaue);
                }
            }
        });
